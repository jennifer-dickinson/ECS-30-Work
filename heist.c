...
void _knapsack(item *items, int num_items, int max_items, int max_weight, int high_val, int *highest_val) {
    // execution of _knapsack occurs in outter function knapsack()

    if (max_weight < 0) {
        // The bag is over capacity, end the recursion, do not update value
        return;
    }
    ...
    for (int i = 0; i < num_items; i++) {
        ...
        _knapsack(items + i + 1, num_items - (i + 1), max_items, max_weight - items[i].weight,
                  high_val + items[i].value, highest_val);
        // param 1 - move items list pointer to the (i + 1) position
        // param 2 - decrease number of items by (i + 1)
        // param 3 - maximum number of items remains the same
        // param 4 - decrease total carrying capacity
        // param 5 - increase value of the contents in the bag
...
double knapsack(int num_items, item *items, int max_weight) {
    ...
    // max_weight: maximum carrying capacity the bag can hold
    // highest_val: the highest value of contents in the bag
...
void userPrompt() {
    ...
    printf("The most profitable heist will net you $%d in total.\n", profit);
